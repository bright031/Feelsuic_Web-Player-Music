...
client = MongoClient("mongodb+srv://dqddong1710_db_user:123@cluster0.a3knegz.mongodb.net")
...
def register(request):
    try:
        ...
        if db['recommend_userprofile'].find_one({"username": username}):
            return Response({'message': 'Tên đăng nhập đã tồn tại'}, status=400)

        # Kiểm tra email tồn tại
        if email and db['recommend_userprofile'].find_one({"email": email}):
        ...
        db['recommend_userprofile'].insert_one(new_user)
...
def login_view(request):
    logger.info(f"Dữ liệu đăng nhập nhận được: {request.data}")
    try:
        username = request.data.get('username')
        password = request.data.get('password')

        if not username or not password:
            logger.info("Thiếu username hoặc password")
            return Response({'message': 'Vui lòng nhập username và password!'}, status=status.HTTP_400_BAD_REQUEST)

        # Tìm user trong MongoDB
        user = db['recommend_userprofile'].find_one({'username': username})
        if not user:
            logger.info(f"Người dùng không tồn tại: {username}")
            return Response({'message': 'Người dùng không tồn tại!'}, status=status.HTTP_400_BAD_REQUEST)

        # Kiểm tra mật khẩu
        if not bcrypt.checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
            logger.info(f"Mật khẩu sai cho user: {username}")
            return Response({'message': 'Mật khẩu sai!'}, status=status.HTTP_400_BAD_REQUEST)

        user_id = str(user['_id'])  # Lấy ObjectId và convert sang string
        ...
        logger.info(f"Đăng nhập thành công cho user: {username}, user_id: {user_id}")
        return Response({
        ...
        return Response({'message': 'Lỗi server: ' + str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def add_historysong(request, user_id):
    try:
        ...
        user = UserProfile.objects(id=user_id).first()  # ✅ dùng id
        if not user:
        ...
        logger.error(f"Lỗi khi thêm bài hát vào lịch sử: {e}")
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def add_historylist(request, user_id):
    try:
        ...
        logger.error(f"Lỗi khi thêm danh sách vào lịch sử: {e}")
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_historysongs(request, user_id):
    try:
        ...
        logger.error(f"Lỗi khi lấy lịch sử bài hát: {e}")
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_historylists(request, user_id):
    try:
        ...
        logger.error(f"Lỗi khi lấy lịch sử danh sách: {e}")
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_login_history(request, user_id):
    try:
        if not user_id or user_id in ['None', 'null', 'undefined']:
            ...
            return Response({'error': 'Invalid user_id'}, status=status.HTTP_400_BAD_REQUEST)
        user = UserProfile.objects.filter(id=ObjectId(user_id)).first()
        if not user:
        ...
        logger.error(f"Lỗi khi lấy lịch sử đăng nhập cho user {user_id}: {e}")
        return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def update_user(request, user_id):
    try:
        ...
        result = UserProfile.objects(id=ObjectId(user_id)).update(**update_data)
        if result == 0:
